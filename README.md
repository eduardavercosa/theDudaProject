...
- Create a copy of ``duda/settings/local.py.example``:  
  `cp duda/settings/local.py.example duda/settings/local.py`
...
- Deregister the example app by removing `'exampleapp.apps.ExampleappConfig'` from ``backend/duda/settings/base.py``.
- Adjust ``backend/duda/urls.py`` to point to your newly created Django app and remove the path configuration that redirects to the deleted example app.
